...
namespace System.Data.Unqlite.Interop
{
    ...
    internal sealed class UnmanagedLibrary : IDisposable
    {
        ...
        public UnmanagedLibrary(string fileName)
        {
            ...
            _systemFileName = fileName + archSuffix + Platform.LibSuffix;
            _handle = LoadFromSystemPath();

            Tracer.InfoIf(_handle != null, "Loading " + _systemFileName + " from LoadLibrary system path.",
                TraceCategory);

            if (_handle == null)
            {
                // Ensure the correct manifest resource will always be used, even if it has already been extracted
                _extractedFileName = fileName + "-" + CurrentArch + "-" +
                                     Assembly.GetExecutingAssembly().GetName().Version + Platform.LibSuffix;
            ...
            if (_handle == null || _handle.IsInvalid)
            {
                throw new FileNotFoundException(
                    "Unable to find " + _systemFileName +
                    " on system path or extract it from assembly manifest resources. Inspect Trace output for more details.",
                    _systemFileName,
        ...
        {
            var p = Platform.LoadProcedure(_handle, functionName);

            if (p == IntPtr.Zero)
            {
                throw new MissingMethodException("Unable to find function '" + functionName +
                                                 "' in dynamically loaded library.");
            }
        ...
        private SafeLibraryHandle LoadFromTempPath()
        {
            ...
            var dir = Path.Combine(Path.GetTempPath(), assemblyName.Name + "-" + assemblyName.Version);
        ...
        {
            var libPath = Path.GetFullPath(Path.Combine(dir, _extractedFileName));
            var platformSuffix = "." + CurrentArch;
